...
from kin_poly.core import LoggerRL, TrajBatch
from kin_poly.utils.memory import Memory
        ...
        while logger.num_steps < min_batch_size:
            ...
            for t in range(10000):
                ...
                if self.custom_reward is not None:                    
                    """available during the fine-tuning, to compute the action of pretrained model and fine-tuned model"""
                    if self.pretrain_policy_net is not None:
                        #old_action = self.pretrain_policy_net.select_action(self.trans_old_policy(state_var), True)[0].numpy()
                        new_action = self.policy_net.select_action(vs_out.clone(), True)[0].numpy()
                        old_action = self.pretrain_policy_net.select_action(vs_out.clone(), True)[0].numpy().astype(np.float64)
                        c_reward, c_info = self.custom_reward(self.env, state, new_action, old_action, info)
                    else:
