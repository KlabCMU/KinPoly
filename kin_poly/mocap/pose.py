...
from kin_poly.utils.transformation import quaternion_slerp, quaternion_from_euler, euler_from_quaternion
import os
...
def load_obj_bvh_file(fname):
    ...
    skip_line = 9
    #skip_line = 54
    data = np.genfromtxt(fname, dtype=float, delimiter=' ', skip_header=skip_line)[:, :6]
    data[:, :3] *= len_scale
    data[:, 3:] = np.deg2rad(data[:, 3:])

    f = open(fname)
    frameRate = float(f.readlines()[skip_line - 1].split(':')[1].rstrip(os.linesep))
    
    return data, round(1.0 / frameRate)

def load_obj_position(fname):
    len_scale = 0.0254
    data = np.genfromtxt(fname, dtype=float, delimiter=' ', skip_header=9)[:, :3]
    data[:, :] *= len_scale
