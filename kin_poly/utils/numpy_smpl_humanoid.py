...
from kin_poly.utils.transform_utils import *
from kin_poly.utils.transformation import quaternion_from_euler_batch, \
    quaternion_multiply_batch, quat_mul_vec, quat_mul_vec_batch, quaternion_from_euler, \
        quaternion_multiply
...
class Humanoid:
    def __init__(self, model_file):
        ...
        offsets = self.model.body_pos[1:25] # exclude world and chair
        i_offsets = self.model.body_ipos[1:25] # exclude world and chair
        parents = self.model.body_parentid[1:25] - 1
        parents[0] = -1
        ...
        self.body_name = self.model.body_names[1:25]
    ...
    def get_body_occup_map(self, qpos, body_name):
        ...
        body_pos, body_quat = self.qpos_fk_batch(torch.unsqueeze(qpos, axis=0), select_joints = body_name)
        ...
        body_rot = torch.stack([self.to4x4(quaternion_matrix(get_heading_q(b_quat))) for b_quat in body_quat], axis=0)
        body_rot[:, :3, 3] = body_pos

        ## Body -> object root transformation
        body_trans = torch.einsum("bij, bjk -> bik", torch.repeat_interleave(torch.unsqueeze(obj_rot, axis=0), len(body_name), axis=0), body_rot)
        

        ## Object root -> object part transformation
        obj_loc_trans = torch.stack([self.to4x4(quaternion_matrix(quat).T) for quat in self.obj_loc_quat], axis=0)
        obj_loc_trans[:, :3, 3] = -self.obj_loc_pos
        
        ## Body -> object part transformation
        total_trans = torch.einsum("blij, bljk -> blik", torch.repeat_interleave(torch.unsqueeze(obj_loc_trans, axis=1), len(body_name), axis=1), \
                                                         torch.repeat_interleave(torch.unsqueeze(body_trans, axis=0), self.obj_geom_num, axis=0))

        grid_h = torch.cat((grid.view(tot_num, 3).t(), torch.ones(1, tot_num, dtype=self.dtype, device=self.device)), axis=0)
        trans_grid = torch.einsum("bkij,jl->bkil", total_trans, grid_h)[:, :, :-1, :] ## object part, body num, xyz1, point num
        obj_sizes = torch.repeat_interleave(torch.repeat_interleave(torch.unsqueeze(torch.unsqueeze(self.obj_sizes, axis=1), axis=-1), len(body_name), axis=1), tot_num, axis=3)
        cond = torch.abs(trans_grid) < obj_sizes / 2.0
        occup_grid_batch = cond.all(axis=2).any(axis=0).view(occup_grid_size).type(self.dtype).to(self.device)
    ...
    def qpos_fk_batch(self, qpos, select_joints=None, fix_quat = True):
        ...
        body_quats = quaternion_from_euler_batch(padded_rot[:, 0], padded_rot[:, 1], padded_rot[:, 2], axes='rzyx')
        ...
        ''' Get sim full vals'''
        body_quats_full = quaternion_from_euler_batch(padded_rot[:, 0], padded_rot[:, 1], padded_rot[:, 2], axes = "sxyz")
        body_quats_full = np.concatenate((root_rotations[:, None, :], body_quats_full.reshape(B, J, 4)), axis = 1)
        ...
        if select_joints is None:
            ...
            wbody_quat  = np.concatenate(wbody_quat_list).reshape(B, len(select_joints), 4)
    ...
    def qpos_fk(self, qpos, select_joints=None, fix_quat = True):
        ...
        body_quats = quaternion_from_euler_batch(rotations[:, 0], rotations[:, 1], rotations[:, 2], axes='rzyx')
        
        wbody_pos, body_com, wbody_quat = self.forward_kinematics(body_quats, root_rotations, root_positions)

        ''' Get sim full vals'''
        body_quats_full = quaternion_from_euler_batch(rotations[:, 0], rotations[:, 1], rotations[:, 2], axes = "sxyz")

        body_quats_full = np.concatenate((root_rotations[None, ], body_quats_full.reshape(J, 4)), axis = 0)
    ...
    def forward_kinematics(self, rotations, root_rotations, root_positions):
        ...
        for i in range(self._offsets.shape[0]):
            if self._parents[i] == -1:
                ...
                positions_world_com.append(root_positions + quat_mul_vec(root_rotations, self._i_offsets[ 0, :]))
            else:
                
                jpos = quat_mul_vec(rotations_world[self._parents[i]], self._offsets[i, :]) +  positions_world[self._parents[i]]
                jquat = quaternion_multiply(rotations_world[self._parents[i]], rotations[i - 1, :])

                positions_world.append( jpos)
                rotations_world.append(jquat)
                positions_world_com.append(quat_mul_vec(jquat, self._i_offsets[i, :]) + jpos) 

        return np.stack(positions_world), np.stack(positions_world_com), np.stack(rotations_world)
    ...
    def forward_kinematics_batch(self, rotations, root_rotations, root_positions):
        ...
        expanded_offsets = np.broadcast_to(self._offsets, (B, J, self._offsets.shape[1]))
        expanded_i_offsets = np.broadcast_to(self._i_offsets, (B, J, self._i_offsets.shape[1]))
        ...
        return positions_world.transpose(0, 2, 1), positions_world_com.transpose(0, 2, 1), rotations_world.transpose(0, 2, 1)
...
def get_expert(expert_qpos):
    ...
    [[fk_res[k].append(v) for k, v in env_humanoid.qpos_fk(qpos).items()] for qpos in expert_qpos]
    fk_res = {k: np.stack(v) for k, v in fk_res.items()}

    ## Batch
    # fk_res = env_humanoid.qpos_fk_batch(expert_qpos)
    print(np.sum(np.abs(wbody_pos_mujoco - fk_res['wbpos'])), \
...
if __name__ == "__main__":
    import argparse
    from kin_poly.data_loaders.statereg_dataset import Dataset
    from kin_poly.envs.humanoid_v2 import HumanoidEnv
    from kin_poly.utils.egomimic_config import Config as EgoConfig
    ...
    cfg = Config("uhc_5", "/insert_directory_here/", create_dirs=False, \
            mujoco_path = "%s.xml"
        )
    ...
    data_load = joblib.load("/insert_directory_here/kin_poly_mocap_qpos_grad.pkl")
