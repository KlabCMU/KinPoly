...
class TrajARNet(nn.Module):
    ...
    def __init__(self, cfg, data_sample, device, dtype, mode = "train"):
        ...
        # Netural data
        self.netural_data = joblib.load("/hdd/zen/data/ActBound/AMASS/standing_neutral.pkl")
    ...
    def get_context(self, data):
        ...
        return torch.cat(data_acc, dim = 2).to(self.device).type(self.dtype)
    ...
    def get_context_feat(self, data):
        context_feat = self.get_context(data)
    ...
    def get_obs(self, data, t):
        ...
        return obs, {"pred_w_pos": pred_wbpos, "pred_w_rot": pred_wbquat,  "pred_rot": pred_bquat,
    ...
    def step(self, action, dt = 1/30):
        ...
        if self.cfg.has_z:
            next_qpos = torch.cat([curr_pos[:,:2], action[:, :self.qpos_lm]], dim=1)
            root_qvel = action[:, self.qpos_lm:]
            linv = quat_mul_vec_batch(curr_heading, root_qvel[:, :3]) 
            next_qpos[:, :2] += linv[:, :2] * dt
        else:
            next_qpos = torch.cat([curr_pos[:,:3], action[:, :self.qvel_lm]], dim=1)
    ...
    def compute_loss(self, feature_pred, data):
        ...
        pred_w_pos = feature_pred['pred_w_pos'].reshape(b_size * seq_len, -1)
        gt_w_pos = data['wbpos'].reshape(b_size *  seq_len, -1)
        # pred_w_rot = feature_pred['pred_w_rot'].view(b_size, seq_len, -1)
        ...
        ee_loss = end_effector_pos_loss(gt_w_pos, pred_w_pos).mean() # End effector loss
