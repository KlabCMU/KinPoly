...
def world_rfc_implicit_v1_mul(env, state, action, info):
    ...
    # overall reward
    reward =  pose_reward * vel_reward *  ee_reward  * com_reward  * vf_reward
    # reward /= w_p + w_v + w_e + w_c + w_vf
    return reward, np.array([pose_reward, vel_reward, ee_reward, com_reward, vf_reward])
...
def world_rfc_implicit_v2(env, state, action, info):
    ...
    k_p, k_wp, k_v, k_j, k_c, k_vf, jpos_diffw = \
        ws.get('k_p', 0.4), ws.get("k_wp", 0.4), ws.get('k_v', 0.005), ws.get('k_j', 100), ws.get('k_c', 100), \
        ws.get('k_vf', 1), ws.get("jpos_diffw", [1] * 24)
    ...
    # overall reward
    reward = pose_reward * wpose_reward  * com_reward * jpos_reward * vel_reward * vf_reward 
    ...
    return reward, np.array([pose_reward, wpose_reward, com_reward, jpos_reward, vel_reward, vf_reward])
...
def world_rfc_implicit_v3(env, state, action, info):
    ...
    cfg = env.cfg
    ws = cfg.reward_weights
    w_p, w_wp, w_v, w_j, w_c, w_vf = ws.get('w_p', 0.4), ws.get("w_wp", 0.4), ws.get('w_v', 0.005), \
         ws.get('w_j', 100), ws.get('w_c', 100), ws.get('w_vf', 1)
    ...
    # overall reward
    reward = w_p * pose_reward + w_wp * wpose_reward  +  w_c *com_reward +  w_j *jpos_reward +  w_v *vel_reward +  w_vf *vf_reward 
    # np.set_printoptions(precision=5, suppress=1)
    # print(np.array([reward, vel_dist, pose_diff, wpose_diff]), \
    # np.array([pose_reward, wpose_reward, com_reward, jpos_reward, vel_reward, vf_reward]))
    return reward, np.array([pose_reward, wpose_reward, com_reward, jpos_reward, vel_reward, vf_reward])
