...
class Config:
    ...
    def __init__(self, cfg_id, base_dir = "", create_dirs=False, cfg_dict=None, mujoco_path = '%s.xml'):
        self.id = cfg_id
        ...
        self.cfg_dir = '%s/motion_im/%s' % (self.result_dir, cfg_id)
        ...
        self.actor_type = cfg.get("actor_type", "gauss")
        if self.actor_type == "mcp":
            self.num_primitive = cfg.get("num_primitive", 8)
            self.composer_dim = cfg.get("composer_dim", [[300, 200]])
        ...
        self.action_v = cfg.get('action_v', 0)
        self.reactive_v = cfg.get('reactive_v', 0)
        self.no_root = cfg.get('no_root', False)
        self.reactive_rate = cfg.get('reactive_rate', 0.3)
        self.sampling_temp = cfg.get("sampling_temp", 2)

        # virutual force
        self.residual_force = cfg.get('residual_force', False)
        self.residual_force_scale = cfg.get('residual_force_scale', 200.0)
        self.residual_force_lim = cfg.get('residual_force_lim', 100.0)
