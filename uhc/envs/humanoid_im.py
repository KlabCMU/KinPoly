...
class HumanoidEnv(mujoco_env.MujocoEnv):
    ...
    def set_spaces(self):
        ...
        if cfg.meta_pd:
            ...
            self.meta_pd_dim =  2 * cfg.jkp.shape[0]
    ...
    def get_full_obs_v2(self):
        ...
        r_quat = cur_quat.copy()
        for i in range(r_quat.shape[0]):
            r_quat[i] = quaternion_multiply(quaternion_inverse(hq), r_quat[i]) # ZL: you have gotta batch this.....
        obs.append(r_quat.ravel()) # obs: current target body quaternion (1, 96) # this contains redundent information 

        rel_quat = np.zeros_like(cur_quat)
        for i in range(rel_quat.shape[0]):
            rel_quat[i] = quaternion_multiply(quaternion_inverse(cur_quat[i]), target_quat[i]) # ZL: you have gotta batch this.....
    ...
    def compute_torque(self, ctrl, i_iter = 0):
        ...
        if cfg.meta_pd:
            ...
            curr_jkp = cfg.jkp.copy() * np.clip((meta_pds[i_iter] + 1), 0, 10)
            curr_jkd = cfg.jkd.copy() * np.clip((meta_pds[i_iter + self.sim_iter] + 1), 0, 10)
            if flags.debug: 
                # import ipdb; ipdb.set_trace()
                print((meta_pds[i_iter + self.sim_iter] + 1), (meta_pds[i_iter] + 1))
        elif cfg.meta_pd_joint:
            num_jts = cfg.jkp.shape[0]
            meta_pds = ctrl[(self.ndof + self.vf_dim):(self.ndof + self.vf_dim + self.meta_pd_dim)]
            curr_jkp = cfg.jkp.copy() * np.clip((meta_pds[:num_jts] + 1), 0, 10)
            curr_jkd = cfg.jkd.copy() * np.clip((meta_pds[num_jts:] + 1), 0, 10)
            if flags.debug: 
                import ipdb; ipdb.set_trace()
        else:
            curr_jkp = cfg.jkp.copy()
            curr_jkd = cfg.jkd.copy()
    ...
    def step(self, a):
        ...
        if cfg.env_term_body == 'Head':
            ...
            fail = self.expert is not None and self.get_humanoid_qpos()[2] < self.expert['height_lb'] - 0.1
        ...
        end =  (self.cur_t >= cfg.env_episode_len) or (self.cur_t + self.start_ind >= self.expert['len'] + cfg.env_expert_trail_steps)
        done = fail or end
